...
else if($_REQUEST['macPhoto_id'] != '')
{
    ...
    if($_REQUEST['status'] == 'ON')
    {
      $photoImg = $wpdb->query("UPDATE " . $wpdb->prefix . "picaphotos SET macPhoto_status='ON' WHERE macPhoto_id='$macPhoto_id'");
    ...
    {
        $photoImg = $wpdb->query("UPDATE " . $wpdb->prefix . "picaphotos SET macPhoto_status='OFF' WHERE macPhoto_id='$macPhoto_id'");
...
if(isset($flag)){     // FOR FEATURED IMAGE
	...
	// $wpdb->query("UPDATE " . $wpdb->prefix . "picaphotos SET macFeaturedCover = $albumCover WHERE macPhoto_id='$macPhotoid' and macAlbum_id='$albumId'");
	
	    echo "<img src='$site_url/wp-content/plugins/$folder/images/tick.png' style='cursor:pointer' width='16' height='16' onclick=macPhoto_status(0,$macPhoto_id)  />";
...
else if($_REQUEST['macPhoto_name'] != '')
{
    ...
    // echo "UPDATE " . $wpdb->prefix . "picaphotos SET `macPhoto_name` = '$macPhoto_name' WHERE `macPhoto_id` = $macPhoto_id";
    // exit;
     echo $macPhoto_name;
}
