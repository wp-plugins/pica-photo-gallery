...
function widget($args, $instance) {
            ...
            $sql = "SELECT macAlbum_id, macAlbum_image ,macAlbum_name , ID FROM  $albTableName , $postTable WHERE macAlbum_status = 'ON' AND is_delete = 0  AND `macAlbum_image` != '' AND post_content = '[picaGallery]' ORDER BY 1 DESC ";
            $albumList = $wpdb->get_results($sql , ARRAY_A );
            $siteUrl = get_option('siteurl');
            $postIdIs = $albumList[0]['ID'];
       //   echo "<pre>"; print_r($albumList); echo "<pre>";      
